...
public class Graph {
    ...
    private final int V;      // מספר קודקודים          // number of vertices in this digraph
    private int E;            // מספר הקצבות          // number of edges in this digraph
    private Bag<DirectedEdge>[] adj; //השכנים של הקודקוד שעליו מדובר     // adj[v] = adjacency list for vertex v
    private int[] indegree;             // indegree[v] = indegree of vertex v
    private Queue<Double> q = new LinkedList<Double>() ; // תור של דאבלים שמאוכסן בתוך לינקדליסט
    /**
     ...
     */
    public Graph(int V) {
    	// שואל האם מספר הקודרודים קטן מ0 שיזרוק אקספשיאן 
        if (V < 0) throw new IllegalArgumentException("Number of vertices in a Digraph must be nonnegative");
        // אחרת תעדכן משתנה מחלקות בבנאי זה 
        this.V = V;
        this.E = 0;
        this.indegree = new int[V];
        adj = (Bag<DirectedEdge>[]) new Bag[V];// משתנה שמכיל לתוכו את המסלול מנקודה מוצא  לנקודה יעד 
        for (int v = 0; v < V; v++)
            adj[v] = new Bag<DirectedEdge>();
        // השמה של כל המקודות לתוך המשתנה ADJ את כל הנקודות 
    }
